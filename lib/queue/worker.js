...
Worker.prototype.delayed = function (job, err, delay, fn) {
  job.failed( function(){
    job
      .delay(delay)
      .update()
...
Worker.prototype.process = function (job, fn) {
    ...
    job.active( function(){
        fn(
            job,
            function (err, result, delay) {
                if (delay) {
                    return self.delayed(job, err, delay, fn);
                } else
                if (err) {
...
Worker.prototype.shutdown = function (fn, timeout) {
    ...
    var _fn = function (job) {
        ...
        (self.type in clients) && clients[self.type].end();
